getgenv().key = "Xana" -- キーを設定（チェックは行わない）
...
spawnToyThread = coroutine.create(
    function()
        while true do
            repeat
...
local function v_u_88_(p85_, p86_, _)
    if p85_ == "Spawn Toy (TAB)" and p86_ == Enum.UserInputState.Begin then
        local v87_ = {_G.SelectedToy, v_u_17_.Character.CamPart.CFrame, Vector3.new(0, v_u_17_.Character.CamPart.Orientation.Y, 0)}
...
function teleportfunc()
    ...
    local v91_, v92_ = v_u_7_:FindPartOnRayWithIgnoreList(
...
function CheckNetworkOwnerShipOnPlayer(p114_, p115_)
    if typeof(p114_) == "Instance" and (p114_:IsA("Player") and p114_.Character) and
        (p114_.Character:FindFirstChild("Head") and
            (p114_.Character.Head:FindFirstChild("PartOwner") and p114_.Character.Head.PartOwner.Value == v_u_17_.Name)) then
        return not p115_ and true or p114_.Character.Head.PartOwner
    end
end

function CheckNetworkOwnerShipPermanentOnPlayer(p116_, p117_)
    if typeof(p116_) == "Instance" and (p116_:IsA("Player") and p116_.Character) and
        (p116_.Character:FindFirstChild("HumanoidRootPart") and
            (p116_.Character.HumanoidRootPart:FindFirstChild("FirePlayerPart") and
                (p116_.Character.HumanoidRootPart.FirePlayerPart:FindFirstChild("PartOwner") and
                    p116_.Character.HumanoidRootPart.FirePlayerPart.PartOwner.Value == v_u_17_.Name))) then
...
function IsPlayerFloating(p123_)
    if typeof(p123_) == "Instance" and (p123_:IsA("Player") and p123_.Character) and
        (p123_.Character:FindFirstChildOfClass("Humanoid") and p123_.Character:FindFirstChildOfClass("Humanoid").FloorMaterial == Enum.Material.Air) then
        return true
    end
end
...
function SNOWshipPermanentPlayer(p135_, p136_)
    if v_u_17_.Character and
        (v_u_17_.Character:FindFirstChild("HumanoidRootPart") and
            ...
            (p135_.Character:FindFirstChild("HumanoidRootPart") and p135_.Character.HumanoidRootPart:FindFirstChild("FirePlayerPart"))) then
...
function GetPlayerCharacter()
    if v_u_17_.Character and
        (v_u_17_.Character:FindFirstChild("HumanoidRootPart") and v_u_17_.Character:FindFirstChildOfClass("Humanoid")) then
...
function setanchorObject(p164_)
    if typeof(p164_) == "Instance" and p164_.Parent and (p164_.Parent:IsA("Model") or p164_.Parent:IsA("Folder")) then
        ...
        if v166_ or v167_ then
            ...
            v171_[1] = v_u_165_.DescendantAdded:Connect(
                function(p182_)
            ...
            v171_[2] = v_u_165_.DescendantRemoving:Connect(
                function(p183_)
    ...
    while true do
        ...
        if v245_.primaryPart then
            ...
            while true do
                ...
                if v250_ and (v250_ == p240_ and (v250_:GetAttribute("Glue") or v250_:GetAttribute("GluePrimary"))) and
                    not v250_:GetAttribute("IsAnchored") then
                    v244_ = true
                    break
                end
            end
...
local function v_u_298_()
    ...
    elseif v279_ == 1 then
        ...
        if v_u_282_ then
            ...
            local v297_ = v_u_14_.Heartbeat:Connect(
                function()
                    v_u_201_(v_u_282_)
                end
            )
...
controltween1 = v_u_9_:Create(v_u_7_.CurrentCamera, TweenInfo.new(0.3, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, 0, true), {["FieldOfView"] = 120})
controltween2 = v_u_9_:Create(controleffectsatur, TweenInfo.new(0.3, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {["TintColor"] = Color3.fromRGB(210, 218, 255)})
controltween3 = v_u_9_:Create(controleffectsatur, TweenInfo.new(1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {["Brightness"] = -0.1})
controltween4 = v_u_9_:Create(controleffectsatur, TweenInfo.new(0.3, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {["TintColor"] = Color3.new(1, 1, 1), ["Brightness"] = 0})
...
function controlcreatureeffectIn()
    ...
    controltween2.Completed:Once(function() controltween3:Play() end)
end

function controlcreatureeffectOut()
    controltween4:Play()
    controltween4.Completed:Once(function() controleffectsatur.Enabled = false end)
end
...
function controlCreature(p_u_315_)
    if typeof(p_u_315_) == "Instance" and p_u_315_:IsA("Model") then
        ...
        local v_u_320_ = (function()
            if not v_u_5_:GetPlayerFromCharacter(p_u_315_) and
                (p_u_315_.Name == "YouDecoy" or (p_u_315_.Name == "CreatureBlobman" or tostring(p_u_315_.Parent.Name) == "Robloxians")) then
                return true
            end
        end)()
        if v_u_316_ and (v_u_317_ and (v318_ or nil)) then
            ...
            v_u_321_[1] = v_u_317_.Died:Connect(function() _G.ControllingCreature = nil end)
            ...
            if v329_ then
                ...
                v328_.Parent = v330_
                v_u_321_[2] = v_u_332_.Died:Connect(function() _G.ControllingCreature = nil end)
                v_u_321_[3] = v_u_10_.JumpRequest:Connect(function() v_u_317_:ChangeState("Jumping") end)
                v_u_321_[5] = v_u_332_.Changed:Connect(
                    function(p333_)
                ...
                v_u_321_[6] = workspace.CurrentCamera.Changed:Connect(
                    function(p334_)
                ...
                local v_u_335_ = nil
                v_u_321_[7] = v_u_319_.Changed:Connect(
                    function(p336_)
...
CharacterRaycastFilter.FilterDescendantsInstances = {GetPlayerCharacter()}
    ...
    if v337_ then
        ...
        if v341_ and (v340_ and v340_.Health > 0) then
            ...
            if v342_:FindFirstChildOfClass("Humanoid") then
                -- プレミアムチェックを削除し、全ユーザーがプレイヤーをコントロール可能に
...
function DevJoinEffect()
    ...
    v_u_9_:Create(v358_, TweenInfo.new(5), {["Brightness"] = 0}):Play()
...
local function v_u_374_(p368_, p369_)
    if type(p368_) == "string" and type(p369_) == "string" then
        local v370_ = {["Message"] = p368_, ["FromSpeaker"] = v_u_5_:FindFirstChild(p369_)}
        ...
        while task.wait(1) do
            ...
            while true do
                ...
                if v_u_379_ ~= v_u_17_ and (v_u_51_(v_u_379_) and not v_u_379_:GetAttribute("Inject")) then
                    ...
                    v_u_38_[v_u_379_.Name] = {["AntiGrab"] = true, ["Protection"] = true}
...
COAroundPParams.FilterDescendantsInstances = {GetPlayerCharacter(), v_u_7_.Map, v_u_7_.Plots, v_u_7_.Waypoints, v_u_7_.Slots}
...
function CheckObjectsAroundPlayer()
    ...
    if v408_ then
        ...
        local function v414_(p411_)
            if not p411_:IsDescendantOf(v_u_7_.Map) and
                ...
                (p411_.Parent:IsA("Model") and
                    (p411_.Parent:FindFirstChildOfClass("BasePart") or
                        (p411_.Parent:FindFirstChildOfClass("Part") or p411_.Parent:FindFirstChildOfClass("MeshPart")))) then
...
local function v_u_434_()
    ...
    while true do
        ...
        if v429_.Name == "SprayCanWD" and
            (v429_:FindFirstChild("StickyRemoverPart") and (v429_.PrimaryPart and Getdistancefromcharacter(v429_.PrimaryPart.Position) < 30)) then
    ...
    if not v_u_428_ then
        if v423_ then
            local v430_ = {"SprayCanWD", CFrame.new(v423_.Position.X, v423_.Position.Y, v423_.Position.Z, -0.133750245, -0.471861839, 0.871468484, -3.7252903e-9, 0.879369617, 0.476139903, -0.991015136, 0.0636838302, -0.117615893), Vector3.new(0, 97.69000244140625, 0)}
            SpawnToy(v430_)
        end
        BuyToy:InvokeServer("SprayCanWD")
    end
    if v_u_428_ and v_u_428_:FindFirstChild("StickyRemoverPart") and
        (v_u_428_.StickyRemoverPart:FindFirstChildOfClass("TouchTransmitter") and not v_u_428_:GetAttribute("Connected")) then
        local v_u_432_ = v_u_428_.DescendantAdded:Connect(
            function(p431_)
        ...
        if v456_ then
            if v_u_448_(v456_) then
                ...
                if v457_ and not v459_.Attachment1 then
                    local v460_ = {v456_, v_u_17_.Character}
                    v458_.HoldItemRemoteFunction:InvokeServer(unpack(v460_))
                elseif v457_ and v459_.Attachment1 and
                    (v459_.Attachment1:IsDescendantOf(v_u_17_.Character) and not v458_.EatingSound.IsPlaying) then
                ...
                elseif not v457_ and v459_.Attachment1 and v459_.Attachment1:IsDescendantOf(v_u_17_.Character) then
                    local v461_ = {v456_, CFrame.new(v449_.Position.X, v449_.Position.Y, v449_.Position.Z, -0.133750245, -0.471861839, 0.871468484, -3.7252903e-9, 0.879369617, 0.476139903, -0.991015136, 0.0636838302, -0.117615893), Vector3.new(0, 97.69000244140625, 0)}
                    v458_.DropItemRemoteFunction:InvokeServer(unpack(v461_))
                end
            end
        else
            local v462_ = {"FoodBanana", CFrame.new(508.073517, 67.2614441, -261.901917, -0.133750245, -0.471861839, 0.871468484, -3.7252903e-9, 0.879369617, 0.476139903, -0.991015136, 0.0636838302, -0.117615893), Vector3.new(0, 97.69000244140625, 0)}
            SpawnToy(v462_)
            BuyToy:InvokeServer("FoodBanana")
        end
    end
    if v_u_454_ and v_u_454_:FindFirstChild("HoldPart") and
        (v_u_454_.HoldPart:FindFirstChild("RigidConstraint") and not v_u_454_:GetAttribute("Connected")) then
        local v_u_464_ = v_u_454_.DescendantAdded:Connect(
            function(p463_)
...
function checkHoldFirePart()
    ...
    if not v484_ then
        local v486_ = {"Campfire", CFrame.new(508.073517, 67.2614441, -261.901917, -0.133750245, -0.471861839, 0.871468484, -3.7252903e-9, 0.879369617, 0.476139903, -0.991015136, 0.0636838302, -0.117615893), Vector3.new(0, 97.69000244140625, 0)}
        SpawnToy(v486_)
...
local function v_u_500_()
    ...
    while true do
        ...
        if v495_.Name == "Campfire" and
            (v495_.PrimaryPart and (Getdistancefromcharacter(v495_.PrimaryPart.Position) < 30 and v495_.FirePlayerPart.CanBurn.Value)) then
            v_u_493_ = v495_
        end
    end
    if not v_u_493_ then
        if v488_ then
            local v496_ = {"Campfire", CFrame.new(v488_.Position.X, v488_.Position.Y, v488_.Position.Z, -0.133750245, -0.471861839, 0.871468484, -3.7252903e-9, 0.879369617, 0.476139903, -0.991015136, 0.0636838302, -0.117615893), Vector3.new(0, 97.69000244140625, 0)}
            SpawnToy(v496_)
        end
        BuyToy:InvokeServer("Campfire")
    end
    if v_u_493_ and v_u_493_:FindFirstChild("FirePlayerPart") and
        (v_u_493_.FirePlayerPart:FindFirstChild("CanBurn") and not v_u_493_:GetAttribute("Connected")) then
        local v_u_498_ = v_u_493_.DescendantAdded:Connect(
            function(p497_)
...
function CheckFakeAim()
    ...
    while true do
        ...
        if v516_.Name == "DiceSmall" and
            (v516_:FindFirstChild("Center") and (v516_.PrimaryPart and Getdistancefromcharacter(v516_.PrimaryPart.Position) < 30)) then
            v_u_515_ = v516_
        end
    end
    if not v_u_515_ then
        if v510_ then
            local v517_ = {"DiceSmall", CFrame.new(v510_.Position.X, v510_.Position.Y, v510_.Position.Z, -0.133750245, -0.471861839, 0.871468484, -3.7252903e-9, 0.879369617, 0.476139903, -0.991015136, 0.0636838302, -0.117615893), Vector3.new(0, 97.69000244140625, 0)}
            SpawnToy(v517_)
        end
        BuyToy:InvokeServer("DiceSmall")
    end
    if v_u_515_ and (v_u_515_:FindFirstChild("Center") and not v_u_515_:GetAttribute("Connected")) then
        local v_u_519_ = v_u_515_.DescendantAdded:Connect(
            function(p518_)
                ...
                function()
                    while v_u_521_.Parent do
                        if v_u_17_.Character and v_u_17_.Character:FindFirstChild("CamPart") then
                            ...
                            local v528_, v529_ = v_u_7_:FindPartOnRayWithIgnoreList(v_u_525_, {v_u_17_.Character, v_u_19_})
                            v_u_527_ = v529_
                            v_u_526_ = v528_
...
local function v_u_539_()
    ...
    if not v536_ then
        if v_u_19_:FindFirstChild("CreatureBlobman") then
            ...
            local v538_ = {"CreatureBlobman", CFrame.new(v531_.Head.Position), Vector3.new(0, 97.69000244140625, 0)}
            SpawnToy(v538_)
...
local v543_ = v542_:MakeTab({["Name"] = "Combat", ["Icon"] = "rbxassetid://7485051715", ["PremiumOnly"] = false})
LongReachGrab_Player = v542_:MakeTab({["Name"] = "Blobman Grab", ["Icon"] = "rbxassetid://7734058599", ["PremiumOnly"] = false})
local v544_ = v542_:MakeTab({["Name"] = "Invincibility", ["Icon"] = "rbxassetid://7734056608", ["PremiumOnly"] = false})
local v545_ = v542_:MakeTab({["Name"] = "Player", ["Icon"] = "rbxassetid://7743871002", ["PremiumOnly"] = false})
Esp_Tab = v542_:MakeTab({["Name"] = "ESP", ["Icon"] = "rbxassetid://7733774602", ["PremiumOnly"] = false})
local v546_ = v542_:MakeTab({["Name"] = "Explosions", ["Icon"] = "rbxassetid://17837704089", ["PremiumOnly"] = false})
local v547_ = v542_:MakeTab({["Name"] = "Teleport", ["Icon"] = "rbxassetid://7733992829", ["PremiumOnly"] = false})
local v548_ = v542_:MakeTab({["Name"] = "Custom Line", ["Icon"] = "rbxassetid://7734022107", ["PremiumOnly"] = false})
local v549_ = v542_:MakeTab({["Name"] = "Grab Auras", ["Icon"] = "rbxassetid://7733955740", ["PremiumOnly"] = false})
local v550_ = v542_:MakeTab({["Name"] = "Keybinds", ["Icon"] = "rbxassetid://11710306232", ["PremiumOnly"] = false})
local v551_ = v542_:MakeTab({["Name"] = "Loop Players", ["Icon"] = "rbxassetid://7733964640", ["PremiumOnly"] = false})
local v552_ = v542_:MakeTab({["Name"] = "Auto", ["Icon"] = "rbxassetid://7733916988", ["PremiumOnly"] = false})
local v553_ = v542_:MakeTab({["Name"] = "Misc", ["Icon"] = "rbxassetid://7733917120", ["PremiumOnly"] = false})
local v_u_554_ = v542_:MakeTab({["Name"] = "Discord Server", ["Icon"] = "rbxassetid://16570630989", ["PremiumOnly"] = false})
local v555_ = v542_:MakeTab({["Name"] = "Config", ["Icon"] = "rbxassetid://7734053495", ["PremiumOnly"] = false})
v542_:MakeTab({["Name"] = "Premium Info", ["Icon"] = "rbxassetid://7734053495", ["PremiumOnly"] = false})
local v556_ = v542_:MakeTab({["Name"] = "Credits", ["Icon"] = "rbxassetid://7733687281", ["PremiumOnly"] = false})

local v_u_557_ = nil
task.spawn(
    function()
        local v558_, v559_ = pcall(function() return loadstring(game:HttpGet("https://pastebin.com/raw/Q4iUTG48"))() end)
        v_u_557_ = "https://discord.gg/ENnZ4W9eMN"
        local v560_ = v_u_554_:AddSection({["Name"] = "Discord Server"})
        v560_:AddLabel(v_u_557_)
        v560_:AddButton({["Name"] = "Copy Discord Server Link", ["Callback"] = function() setclipboard(v_u_557_) v_u_33_("Copied to your clipboard") end})
        v560_:AddLabel("Join my discord server to see updates!")
    end
)

-- CREDITS SECTION
local CRACKED_CREDITS = v556_:AddSection({["Name"] = "CRACK CREDITS!"})
local v561_ = v556_:AddSection({["Name"] = "1# Medal credits"})
local v562_ = v556_:AddSection({["Name"] = "2# Medal credits"})
local v563_ = v556_:AddSection({["Name"] = "3# Medal credits"})

local v_u_564_ = game:GetService("UserService")
local v_u_565_ = {90063030, 2298910483, 1030559478, 1762306425, 542649826, 237152138, 1390422876, 3089724826, 882860613, 7280113503}
local v566_, v567_ = ypcall(function() return v_u_564_:GetUserInfosByUserIdsAsync(v_u_565_) end)
...
while true do
    ...
    if v583_ == nil then break end
    v581_ = v583_
    if not v572_[v583_] then
        v582_[v583_] = {["DisplayName"] = "deleted", ["Username"] = "deleted"}
    end
end
if v566_ then
    v561_:AddParagraph("Luna! I cracked this hub!")
    v561_:AddParagraph(v572_[1].DisplayName .. " (" .. v572_[1].Username .. ")", "Thanks for made the whole GUI (Combat, Player, Auras and more) XD!")
    v561_:AddParagraph(v572_[2].DisplayName .. " (" .. v572_[2].Username .. ")", "Thanks for inspiration to create the blobman functions, Massless Grab and Line color changer script!")
    v561_:AddParagraph(v572_[3].DisplayName .. " (" .. v572_[3].Username .. ") " .. "and " .. v572_[6].DisplayName .. " (" .. v572_[6].Username .. ")", "Thanks for sharing the Attraction Aura, Silent Aim, Further Extend scripts!")
    v561_:AddParagraph(v572_[7].DisplayName .. " (" .. v572_[7].Username .. ")", "Thanks for helping to fix kick stuff and anti-blobman")
...
perspectiveON_effect1 = v_u_9_:Create(ImageLabel, t1p, {["ImageTransparency"] = imagestransparencyeffect})
perspectiveON_effect2 = v_u_9_:Create(PerspectiveSaturation, t1p, {["Saturation"] = saturationvalue})
perspectiveOff_effect1 = v_u_9_:Create(ImageLabel, t2p, {["ImageTransparency"] = 1})
perspectiveOff_effect2 = v_u_9_:Create(PerspectiveSaturation, t2p, {["Saturation"] = 0})
...
getgenv().Settings = {["Fov"] = 150, ["Hitbox"] = {"Head", "Torso", "Left Leg", "Right Leg"}, ["FovCircle"] = false}
...
local function v_u_622_(_)
    ...
    while true do
        ...
        if v617_.Name ~= v_u_607_.Name and (v617_.Character and (v_u_607_ and v_u_607_.Character)) and
            v_u_607_.Character:FindFirstChild("HumanoidRootPart") then
...
if hookmetamethod then
    ...
    v_u_633_ = hookmetamethod(
        ...
        function(...)
            ...
            if v635_ == workspace and not checkcaller() and v636_ == "Raycast" and v_u_623_ and v_u_623_.Character and
                v_u_623_.Character:FindFirstChild("HumanoidRootPart") and
                v_u_607_ and v_u_607_.Character and v_u_607_.Character:FindFirstChild("HumanoidRootPart") and
                v_u_623_.Character:FindFirstChild("Humanoid") and v_u_623_.Character.Humanoid.Health > 0 and
                not v_u_623_:FindFirstChild("InPlot") and _G.SilentAim then
...
-- AUTO SECTION
local v650_ = v552_:AddSection({["Name"] = "Auto Get Coins"})
local v651_ = v552_:AddSection({["Name"] = "Auto Time-Reset"})
local v652_ = v552_:AddSection({["Name"] = "Auto Claim-Plot"})
                ...
                while _G.AutoFarmCoins do
                    if v_u_643_() then
                        ...
                        while true do
                            ...
                            if not v_u_643_() then break end
                        end
                        task.cancel(v655_)
                        newtask = nil
                        v_u_649_("Spin")
        ...
        v_u_7_.Plots.DescendantAdded:Connect(
            function(p668_)
                if p668_.Name == "Value" and (tostring(p668_.Parent) == "ThisPlotsOwners" and p668_.Value == v_u_17_.Name) then
                    ...
                    if RTime then
                        RTime.Changed:Connect(function(p669_) TimeInHouseLabel:Set("Time: " .. p669_) end)
                    end
                end
            end
        )
            ...
            if p676_ then
                while _G.AutoSaveHouseTime do
                    if v_u_17_.InfiniteHouseTime.Value then
                        v_u_675_:Set(false)
                        v_u_4_:MakeNotification({["Name"] = "Stop being greedy!", ["Content"] = "You already own infinity house gamepass!", ["Image"] = "rbxassetid://4483345998", ["Time"] = 5})
                        break
                    end
...
function GetPlotModel(_)
    ...
    if v681_ == "Witch House" then v680_ = v680_:FindFirstChild("Plot3")
    elseif v681_ == "Lumber House" then v680_ = v680_:FindFirstChild("Plot2")
    elseif v681_ == "Common House" then v680_ = v680_:FindFirstChild("Plot1")
    elseif v681_ == "American House" then v680_ = v680_:FindFirstChild("Plot4")
    elseif v681_ == "Chinese House" then v680_ = v680_:FindFirstChild("Plot5") end
    return v680_
end
    ...
    if v682_ then
        ...
        local function v689_()
            ...
            while true do
                ...
                if v686_ == nil then break end
                if v688_.Value == v_u_17_.Name then v687_ = true end
            end
            return v687_
        end
...
function UpdatePlotOwner()
    ...
    while true do
        ...
        if v700_.Name == "PlayerRole" then
            ...
            local function v_u_711_()
                ...
                if v_u_704_ and (v_u_702_:IsDescendantOf(v_u_704_) and (v_u_702_.Text == "Owner" and v_u_703_.Visible)) then
                    ...
                    while true do
                        ...
                        if v709_ == nil then break end
                        if v710_.DisplayName == v_u_701_.Text then v_u_705_ = true end
                    end
            ...
            v_u_702_.Changed:Connect(function(p712_) if p712_ == "Text" then v_u_711_() end end)
            v_u_679_.Changed:Connect(function(_) v_u_711_() end)
            v_u_711_()
        end
    end
end
...
function IsThereOwnerOnPlot()
    ...
    if v713_ and v713_.PlotSign.ThisPlotsOwners:FindFirstChild("Value") then return true end
end
        ...
        if v_u_718_.Name == "ThisPlotsOwners" then
            local function v_u_723_()
                ...
                if v720_ and v_u_718_:IsDescendantOf(v720_) then
                    ...
                    if PlayersInPlot then PlayersInPlot:Set("Players in Plot: " .. v722_) end
                    if v722_ == 0 and PlotOwner then PlotOwner:Set("Plot Available!") end
                end
            end
            v_u_679_.Changed:Connect(function(_) v_u_723_() end)
...
task.spawn(function() UpdatePlotOwner() task.wait() UpdatePeopleInPlot() end)
PlotOwner = v652_:AddLabel("Plot Owner:")
PlayersInPlot = v652_:AddLabel("Players in Plot: 0")
v652_:AddButton({["Name"] = "Claim Plot!", ["Callback"] = function() ClaimPlot() end})
...
if v21_.Value == 200 then snowballMaxAmmount = 40 end
...
function holdOwnership()
    if not _G.SnowbalEffectSpam then return end
    local v737_ = v_u_19_
    ...
    if v742_ and (v742_.Name == "BallSnowball" and v742_:FindFirstChild("SoundPart")) then
        ...
        if not CheckNetworkOwnerShipOnPart(v741_) then
            if not lastpossb then lastpossb = GetPlayerCFrame() end
            for _ = 1, 10 do
...
function CountGrownSnowsballs()
    ...
    while true do
        ...
        if v745_ == nil then break end
        if v745_:IsDescendantOf(v_u_7_) then v746_ = v746_ + 1 else v_u_727_[v745_] = nil end
    end
    return v746_
end
...
function newSnowball(p_u_753_)
    if p_u_753_.Name == "BallSnowball" and _G.SnowbalEffectSpam then
        task.spawn(function() checkSize(p_u_753_) end)
        task.spawn(function() modify(p_u_753_) end)
    end
end

task.spawn(function() while task.wait() do CountGrownSnowsballs() end end)

local v754_ = v546_:AddSection({["Name"] = "Snowball"})
v754_:AddSlider(
    {
        ["Name"] = "Ammount", ["Min"] = 5, ["Max"] = snowballMaxAmmount, ["Default"] = 5,
        ["Color"] = Color3.fromRGB(255, 255, 255), ["Increment"] = 1, ["ValueName"] = "Snowballs you want to make to explode them!",
        ["Callback"] = function(p755_) v_u_728_ = p755_ end, ["Save"] = true, ["Flag"] = "ammountsnowballtomake_slider"
    }
)
...
automakesnowballtoggle = v754_:AddToggle(
    {
        ["Name"] = "Auto Make Snowball", ["Default"] = false,
                    ...
                    function()
                        while _G.SnowbalEffectSpam do
                            if v_u_728_ > countToys("BallSnowball") then
                                SpawnToy({"BallSnowball", CFrame.new(-389, 228, 550, -0.3092496991157532, 0.2610282301902771, -0.9144555330276489, 0, 0.9615919589996338, 0.2744831442832947, 0.9509809017181396, 0.08488383144140244, -0.2973720133304596), Vector3.new(0, 97.69000244140625, 0)})
                                task.wait(0.1)
                            end
                            if v_u_728_ <= CountGrownSnowsballs() then automakesnowballtoggle:Set(false) end
                            task.wait()
                        end
                    end
                )
                task.spawn(function() holdOwnership() end)
                local v757_ = v_u_19_
                local v758_, v759_, v760_ = ipairs(v757_:GetChildren())
                while true do
                    local v761_
                    v760_, v761_ = v758_(v759_, v760_)
                    if v760_ == nil then break end
                    newSnowball(v761_)
                end
        ...
        ["Save"] = true, ["Flag"] = "autofarmsnowball_toggle"
    }
)
...
v754_:AddButton(
    {
        ...
        ["Callback"] = function()
            ...
            while true do
                ...
                if v764_ == nil then break end
                if v764_:IsDescendantOf(v_u_7_) then ExplodeSb(v764_.Parent) end
            end
        end
    }
)
...
canExplode = false
maxBombstoexplode = 8
if v21_.Value == 200 then maxBombstoexplode = 18 end
...
function ExplodeFw()
    ...
    while true do
        ...
        if v769_ == nil then break end
        if v770_.Name == spamexplosiontype then
            local v771_ = {
                {
                    ["Radius"] = 17.5, ["TimeLength"] = 0.5,
                    ["Hitbox"] = v770_:FindFirstChild("PartHitDetector"),
                    ["ExplodesByFire"] = true, ["MaxForcePerStudSquared"] = 225,
                    ["DestroysModel"] = true, ["Model"] = v770_,
                    ["ExplodesByPointy"] = false, ["ImpactSpeed"] = 20,
            ...
            if spamexplosiontarget ~= 0 then
                if spamexplosiontarget ~= 1 then
                    ...
                    if v772_ then v771_[1].PositionPart = v772_ v771_[2] = v772_.Position end
                else
                    local v773_
                    if _G.TargetToBombPlayer then v773_ = v_u_5_:FindFirstChild(_G.TargetToBombPlayer) else v773_ = nil end
                    if v773_ and (not IsPlayerInsideSafeZone(v773_) and v773_.Character) and
                        v773_.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if explosionInterval > 0 then task.wait(explosionInterval) end
    end
end

firework_section = v546_:AddSection({["Name"] = "Explosions Spam"})
explosionexplanation = v546_:AddSection({["Name"] = "FAQ about (Explosions Spam)"})

firework_section:AddToggle(
    {
        ["Name"] = "Explode", ["Default"] = false,
        ["Callback"] = function(p775_)
            ...
            if p775_ then
                task.spawn(
                    function()
                        while _G.FireworkEffectSpam do
                            ...
                            if countToys(spamexplosiontype) < bombsammountoexplode and
                                (not reachedrightammount and (spamexplosiontarget ~= 2 or GetFakeAim())) and v776_ then
                                SpawnToy({spamexplosiontype, CFrame.new(v776_.Position.X, v776_.Position.Y, v776_.Position.Z, -0.3092496991157532, 0.2610282301902771, -0.9144555330276489, 0, 0.9615919589996338, 0.2744831442832947, 0.9509809017181396, 0.08488383144140244, -0.2973720133304596), Vector3.new(0, 97.69000244140625, 0)})
                            end
                            task.wait()
                        end
                    end
                                    ...
                                    if spamexplosiontype ~= "BombDarkMatter" then
                                        if spamexplosiontype ~= "BombMissile" then
                                            if spamexplosiontype ~= "BombBalloon" then
                                                ...
                                                else v782_ = v781_:FindFirstChild("Hitbox") end
                                            else v782_ = v781_:FindFirstChild("Balloon") end
                                        else v782_ = v781_:FindFirstChild("Body") end
                                    else v782_ = v781_:FindFirstChild("Pyramid") end
                        ...
                        while _G.FireworkEffectSpam do
                            if countToys(spamexplosiontype) < bombsammountoexplode then
                                ...
                                if spamexplosiontarget ~= 2 or not _G.FireBomb then
                                    if spamexplosiontarget ~= 2 then canExplode = true end
                                else canExplode = true end
                ...
                task.spawn(
                    function()
                        while _G.FireworkEffectSpam do
                            if spamexplosiontarget == 2 then GetFakeAim2() end
                            wait(0.1)
                        end
                    end
                )
...
firework_section:AddDropdown(
    {
        ["Name"] = "Explosion Type", ["Default"] = "Firework",
        ["Options"] = {"Firework", "Missile", "Void", "Ballon", "Small Present", "Big Present"},
        ["Callback"] = function(p783_)
            if p783_ == "Firework" then spamexplosiontype = "FireworkMissile"
            elseif p783_ == "Missile" then spamexplosiontype = "BombMissile"
            elseif p783_ == "Void" then spamexplosiontype = "BombDarkMatter"
            elseif p783_ == "Ballon" then spamexplosiontype = "BombBalloon"
            elseif p783_ == "Small Present" then spamexplosiontype = "PresentSmall"
            elseif p783_ == "Big Present" then spamexplosiontype = "PresentBig" end
        end
    }
)
...
firework_section:AddSlider(
    {
        ["Name"] = "Ammount to Explode", ["Min"] = 1, ["Max"] = maxBombstoexplode, ["Default"] = 1,
        ["Color"] = Color3.fromRGB(255, 255, 255), ["Increment"] = 1, ["ValueName"] = "to explode the player brutally",
        ["Callback"] = function(p784_) bombsammountoexplode = p784_ end
    }
)

firework_section:AddSlider(
    {
        ["Name"] = "Delay", ["Min"] = 0, ["Max"] = 1, ["Default"] = 0,
        ["Color"] = Color3.fromRGB(255, 255, 255), ["Increment"] = 0.1, ["ValueName"] = "interval between every explosion",
        ["Callback"] = function(p785_) explosionInterval = p785_ end
    }
)

firework_section:AddDropdown(
    {
        ["Name"] = "Target", ["Default"] = "Spawn", ["Options"] = {"Spawn", "Player", "Mouse"},
        ["Callback"] = function(p786_)
            if p786_ == "Spawn" then spamexplosiontarget = 0
            elseif p786_ == "Player" then spamexplosiontarget = 1
            elseif p786_ == "Mouse" then spamexplosiontarget = 2 end
        end
    }
)
...
PlayerToTarget = firework_section:AddDropdown(
    {
        ["Name"] = "Select Player", ["Default"] = "Macaco (negro)", ["Options"] = {""},
...
-- MISC SECTION
local v789_ = v553_:AddSection({["Name"] = "Silent-Aim"})
v789_:AddToggle(
    {["Name"] = "Silent Aim", ["Default"] = false, ["Callback"] = function(p790_) _G.SilentAim = p790_ end,
     ["Save"] = true, ["Flag"] = "SilentAim_toggle"}
)
v789_:AddSlider(
    {["Name"] = "Silent-Aim Range", ["Min"] = 0, ["Max"] = 50, ["Default"] = 50,
     ["Color"] = Color3.fromRGB(255, 255, 255), ["Increment"] = 1, ["ValueName"] = "",
     ["Callback"] = function(p791_) v_u_610_ = p791_ end, ["Save"] = true, ["Flag"] = "silentaimrange_slider"}
)

-- LINE EXTENDER
local v792_ = v548_:AddSection({["Name"] = "Line Extender"})
v792_:AddToggle(
    {["Name"] = "Further Extend", ["Default"] = false, ["Callback"] = function(p793_) _G.FutherExtend = p793_ end,
     ["Save"] = true, ["Flag"] = "FurtherLineExtend_toggle"}
)

MaxExtendLine = 0
MinExtendLine = 0
if v_u_10_.TouchEnabled then MinExtendLine = 3 MaxExtendLine = 25
elseif v_u_10_.MouseEnabled then MinExtendLine = 3 MaxExtendLine = 25 end

v792_:AddSlider(
    {["Name"] = "Increase Extend", ["Min"] = MinExtendLine, ["Max"] = MaxExtendLine, ["Default"] = 3,
     ["Color"] = Color3.fromRGB(255, 255, 255), ["Increment"] = 1, ["ValueName"] = "Ammount",
     ["Callback"] = function(p794_) IncreaseLineExtend = p794_ end, ["Save"] = true, ["Flag"] = "FurtherLineExtend_slider"}
)

-- GRAB AURAS
local v795_ = v549_:AddSection({["Name"] = "Normal Auras"})
local v796_ = v549_:AddSection({["Name"] = "Fling Aura"})
local v797_ = v549_:AddSection({["Name"] = "Kick Aura"})
local v798_ = v549_:AddSection({["Name"] = "Auras Whitelist"})

local function v_u_801_()
    local v799_ = v_u_17_.Character
    local v800_
    if v799_ then v800_ = v799_:FindFirstChildOfClass("Humanoid") else v800_ = nil end
...
local function v_u_808_(p802_)
    ...
    if v_u_801_() and _G.LoopKick then
        ...
        while true do
            ...
            if v806_ == nil then break end
            if p802_ == v807_ then v803_ = true end
        end
    end
    return v803_
end
...
local function v_u_810_(p809_)
    if typeof(p809_) == "Instance" and (p809_ ~= v_u_17_ and (not v_u_51_(p809_) and p809_.Character)) and
        (p809_.Character:IsDescendantOf(v_u_7_) and
            (p809_.Character:FindFirstChild("HumanoidRootPart") and
                (p809_.Character:FindFirstChildOfClass("Humanoid") and p809_.Character.Humanoid.Health > 0))) then
        return true
    end
end

local function v_u_812_(p811_)
    if v_u_810_(p811_) and not IsPlayerInsideSafeZone(p811_) then return true end
end

local function v_u_814_(p813_)
    if v_u_810_(p813_) and not (v_u_97_(p813_.Name) and _G.WhitelistFriends) and not v_u_808_(p813_.Name) and
        not (p813_.Character:GetAttribute("Kicking") or _G.KickAura) then
        return true
    end
end

local function v_u_816_(p815_)
    if v_u_810_(p815_) and not (v_u_97_(p815_.Name) and _G.WhitelistFriends) and not v_u_808_(p815_.Name) and
        not p815_.Character:GetAttribute("Kicking") then
        return true
    end
end

local function v_u_818_(p817_)
    if v_u_810_(p817_) and not (v_u_97_(p817_.Name) and _G.WhitelistFriends3) and not v_u_808_(p817_.Name) and
        not p817_.Character:GetAttribute("Kicking") then
        return true
    end
end
...
local function v_u_822_(p821_)
    if v_u_810_(p821_) and not (v_u_97_(p821_.Name) and _G.WhitelistFriends3) and
        not (IsPlayerInsideSafeZone(p821_) or IsPlayerFloating(p821_)) then
        return true
    end
end
...
v795_:AddToggle(
    {
        ["Name"] = "Poison Aura", ["Default"] = false,
        ...
        ["Save"] = true, ["Flag"] = "poisonaura_toggle"
    }
)

v795_:AddToggle(
    {
        ["Name"] = "Death Aura", ["Default"] = false,
        ...
        ["Save"] = true, ["Flag"] = "deathaura_toggle"
    }
)
...
if v_u_825_ then
    v795_:AddToggle(
        {
            ["Name"] = "Radioactive Aura", ["Default"] = false,
            ...
            ["Save"] = true, ["Flag"] = "radioaura_toggle"
        }
    )
end
...
v795_:AddToggle(
    {
        ...
        ["Save"] = true, ["Flag"] = "burnaura_toggle"
    }
)

v796_:AddToggle(
    {
        ["Name"] = "Fling Aura", ["Default"] = false,
        ...
        ["Save"] = true, ["Flag"] = "flingaura_toggle"
    }
)

v796_:AddSlider(
    {["Name"] = "Strength", ["Min"] = 400, ["Max"] = 10000, ["Default"] = 400,
     ["Color"] = Color3.fromRGB(255, 255, 255), ["Increment"] = 100, ["ValueName"] = "",
     ["Callback"] = function(p883_) _G.FlingStrength = p883_ end, ["Save"] = true, ["Flag"] = "flingstrengthvalue_toggle"}
)

v796_:AddDropdown(
    {["Name"] = "Target", ["Default"] = "Players", ["Options"] = {"Players", "Objects", "Players and Objects"},
     ["Callback"] = function(p884_)
         if p884_ == "Players" then _G.FlingTarget = 1
         elseif p884_ == "Objects" then _G.FlingTarget = 2
         elseif p884_ == "Players and Objects" then _G.FlingTarget = 3 end
     end, ["Save"] = true, ["Flag"] = "flingtarget_dropdown"}
)

v795_:AddToggle(
    {
        ["Name"] = "Attraction Aura", ["Default"] = false,
        ...
        ["Save"] = true, ["Flag"] = "attractaura_toggle"
    }
)
    ...
    if p_u_896_:FindFirstChild("KickAuraP") then
        ...
        task.spawn(
            function()
                ...
                v_u_908_.FilterDescendantsInstances = {p_u_895_}
                ...
                local function v910_(p909_)
                    if p909_ == "Silent" then
                        ...
                        if v_u_902_ then v_u_898_.Position = v_u_902_.Position + Vector3.new(0, 5, 0) end
                ...
                while v_u_898_.Parent and p_u_895_.Parent do
                    ...
                    elseif p_u_897_ ~= "Counter" then
                        ...
                        if not _G.AutoAttacker then break end
...
-- プレミアムチェックを削除したKick Aura
kickauratoggle = v797_:AddToggle(
    {
        ["Name"] = "Kick Aura", ["Default"] = false,
            ...
            if p911_ then
                while _G.KickAura do
                    ...
                    while true do
                        ...
                        if v915_ == nil then break end
                        if v_u_816_(v916_) then
                            local v917_ = v916_.Character
                            local v918_ = v917_:FindFirstChild("HumanoidRootPart")
                            if v918_ and (v917_:FindFirstChildOfClass("Humanoid") and
                                (v918_:FindFirstChild("FirePlayerPart") and SNOWshipPlayer(v916_))) then
...
v797_:AddDropdown(
    {["Name"] = "Kick Type", ["Default"] = "Go to the heaven!", ["Options"] = {"Go to the heaven!"},
     ["Callback"] = function(p919_) _G.KickAuraType = p919_ end, ["Save"] = true, ["Flag"] = "kickauratype_dropdown"}
)

v798_:AddToggle(
    {["Name"] = "Whitelist Friends", ["Default"] = false,
     ["Callback"] = function(p920_) _G.WhitelistFriends = p920_ end, ["Save"] = true, ["Flag"] = "whitelistaura_toggle"}
)

-- COMBAT SECTION
local v921_ = v543_:AddSection({["Name"] = "Strength"})
local v922_ = v543_:AddSection({["Name"] = "Others"})
local v923_ = v543_:AddSection({["Name"] = "Perspective"})

v921_:AddToggle(
    {["Name"] = "Super Strength", ["Default"] = false, ["Callback"] = function(p924_) _G.SuperStrength = p924_ end,
     ["Save"] = true, ["Flag"] = "superstrengthgrab_toggle"}
)

v921_:AddSlider(
    {["Name"] = "Strength", ["Min"] = 400, ["Max"] = 10000, ["Default"] = 400,
     ["Color"] = Color3.fromRGB(255, 255, 255), ["Increment"] = 100, ["ValueName"] = "",
     ["Callback"] = function(p925_) _G.Strength = p925_ end, ["Save"] = true, ["Flag"] = "superstrengthvalue_toggle"}
)

v922_:AddToggle({["Name"] = "Poison Grab", ["Default"] = false, ["Callback"] = function(p926_) _G.Poison_Grab = p926_ end, ["Save"] = true, ["Flag"] = "poisongrab_toggle"})
v922_:AddToggle({["Name"] = "Burn Grab", ["Default"] = false, ["Callback"] = function(p927_) _G.Burn_Grab = p927_ end, ["Save"] = true, ["Flag"] = "burngrab_toggle"})
v922_:AddToggle({["Name"] = "Death Grab", ["Default"] = false, ["Callback"] = function(p928_) _G.Death_Grab = p928_ end, ["Save"] = true, ["Flag"] = "deathgrab_toggle"})
v922_:AddToggle({["Name"] = "Massless Grab", ["Default"] = false, ["Callback"] = function(p929_) _G.MasslessGrab = p929_ end, ["Save"] = true, ["Flag"] = "masslessgrab_toggle"})

if v_u_825_ then
    v922_:AddToggle({["Name"] = "Radiactive Grab", ["Default"] = false, ["Callback"] = function(p930_) _G.Radiactive_Grab = p930_ end, ["Save"] = true, ["Flag"] = "radiactivegrab_toggle"})
end

v922_:AddToggle({["Name"] = "Noclip Grab", ["Default"] = false, ["Callback"] = function(p931_) _G.NoclipGrab = p931_ end, ["Save"] = true, ["Flag"] = "noclipgrab_toggle"})

local v_u_932_ = nil
local v_u_933_ = 50
kickgrabtoggle = nil

v923_:AddToggle({["Name"] = "Perspective Grab", ["Default"] = false, ["Callback"] = function(p934_) _G.PerspectiveGrab = p934_ end, ["Save"] = true, ["Flag"] = "perspectivegrab_toggle"})
v923_:AddSlider(
    {["Name"] = "Speed", ["Min"] = 50, ["Max"] = 150, ["Default"] = 50,
     ["Color"] = Color3.fromRGB(255, 255, 255), ["Increment"] = 1, ["ValueName"] = "",
     ["Callback"] = function(p935_) v_u_933_ = p935_ end, ["Save"] = true, ["Flag"] = "perspectivespeedvalue_toggle"}
)

-- ANNOY PLAYERS SECTION (プレミアムチェック削除済み)
local v936_ = v553_:AddSection({["Name"] = "Annoy Players"})
local v937_ = v553_:AddSection({["Name"] = "Kick All"})
local v938_ = v553_:AddSection({["Name"] = "Whitelist"})
...
annoyalltoggle = v936_:AddToggle(
    {
        ["Name"] = "Ragdoll All", ["Default"] = false,
                        ...
                        if v_u_820_(v962_) then
                            ...
                            if v962_ and (v963_ and v964_) then
                                for _ = 0, 50 do
                                    v_u_349_()
                                    SNOWship(v963_)
                                    if not v_u_820_(v962_) or (not _G.KillAll or
                                        (CheckNetworkOwnerShipOnPlayer(v962_) or v963_.AssemblyLinearVelocity.Magnitude > 500)) then
                        ...
                        if v_u_822_(v970_) then
                            ...
                            if v970_ and v971_ then
                                for _ = 0, 50 do
                                    v_u_349_()
                                    SNOWship(v971_)
                                    if not v_u_822_(v970_) or (not _G.KickAll or
                                        (CheckNetworkOwnerShipOnPlayer(v970_) or v971_.AssemblyLinearVelocity.Magnitude > 500)) then
...
v937_:AddDropdown(
    {["Name"] = "Kick Type", ["Default"] = "Go to the heaven!", ["Options"] = {"Go to the heaven!"},
     ["Callback"] = function(p972_) _G.KickAllType = p972_ end, ["Save"] = true, ["Flag"] = "kickalltype_dropdown"}
)

v938_:AddToggle(
    {["Name"] = "Whitelist Friends", ["Default"] = false,
     ["Callback"] = function(p973_) _G.WhitelistFriends3 = p973_ end, ["Save"] = true, ["Flag"] = "whitelistfriends3_toggle"}
)

-- INVINCIBILITY SECTION
local v974_ = v544_:AddSection({["Name"] = "Invulnerability"})
local v975_ = v544_:AddSection({["Name"] = "Counter-Attack"})
...
v974_:AddToggle(
    {
        ...
        ["Callback"] = function(p976_)
            _G.AntiGrab = p976_
            if p976_ and not v_u_51_(v_u_29_) then v_u_30_:FireServer(v_u_17_) end
        end,
        ["Save"] = true, ["Flag"] = "antigrab_toggle"
    }
)

v974_:AddToggle({["Name"] = "Anti-Burn", ["Default"] = false, ["Callback"] = function(p977_) _G.AntiBurn = p977_ end, ["Save"] = true, ["Flag"] = "antiburn_toggle"})
v974_:AddToggle({["Name"] = "Anti-Explosion", ["Default"] = false, ["Callback"] = function(p978_) _G.AntiExplosion = p978_ end, ["Save"] = true, ["Flag"] = "antiexplosion_toggle"})

v975_:AddToggle({["Name"] = "Auto-Attacker", ["Default"] = false, ["Callback"] = function(p979_) _G.AutoAttacker = p979_ end, ["Save"] = true, ["Flag"] = "rinnegan_toggle"})

counterdropdownselection = v975_:AddDropdown(
    {["Name"] = "Counter Mode", ["Default"] = "Repulsion", ["Options"] = {"Repulsion", "Freeze", "Death", "Kick"},
     ["Callback"] = function(p980_) _G.CounterMode = p980_ end}
)
...
floppatween = v_u_9_:Create(Floppa, floppatweeninfo1, {["Position"] = UDim2.new(0.0185752641, 0, 0.661330521, 0)})
...
v974_:AddToggle(
    {
        ...
        ["Save"] = true, ["Flag"] = "antivoid_toggle"
    }
)

v974_:AddToggle(
    {["Name"] = "Anti-Lag", ["Default"] = false, ["Callback"] = function(p986_) anticreatelinelocalscript.Disabled = p986_ end,
     ["Save"] = true, ["Flag"] = "antilag_toggle"}
)
...
antikicktoggle = v974_:AddToggle(
    {
        ...
        ["Save"] = true, ["Flag"] = "antikick_toggle"
    }
)
...
ESP_Section1 = Esp_Tab:AddSection({["Name"] = "ESP Highlight"})
ESP_Section2 = Esp_Tab:AddSection({["Name"] = "ESP Billboard"})
        ...
        ["Callback"] = function(p988_)
            ...
            if p988_ then
                ...
                local function v_u_991_(p989_)
                    ...
                    if v990_ then v990_.Parent = playersCharFolder end
                end
                local function v997_()
                    ...
                    while true do
                        ...
                        if v995_ == nil then break end
                        v_u_991_(v996_)
                    end
                end
                v997_()
                while _G.ESP_Hightlight do v997_() wait(2) end
...
ESP_Section1:AddColorpicker(
    {["Name"] = "Fill Color", ["Default"] = Color3.fromRGB(255, 0, 0),
     ["Callback"] = function(p998_) highlightesp.FillColor = p998_ end, ["Save"] = true, ["Flag"] = "espHighlightFillcolor_picker"}
)

ESP_Section1:AddSlider(
    {["Name"] = "Fill Transparency", ["Min"] = 0, ["Max"] = 1, ["Default"] = 0.5,
     ["Color"] = Color3.fromRGB(255, 255, 255), ["Increment"] = 0.1, ["ValueName"] = "Fill color transparency:",
     ["Callback"] = function(p999_) highlightesp.FillTransparency = p999_ end, ["Save"] = true, ["Flag"] = "espHighlightFillTransparency_slider"}
)

ESP_Section1:AddColorpicker(
    {["Name"] = "Outline Color", ["Default"] = Color3.fromRGB(255, 0, 0),
     ["Callback"] = function(p1000_) highlightesp.OutlineColor = p1000_ end, ["Save"] = true, ["Flag"] = "espHighlightOutlinecolor_picker"}
)

ESP_Section1:AddSlider(
    {["Name"] = "Outline Transparency", ["Min"] = 0, ["Max"] = 1, ["Default"] = 0.5,
     ["Color"] = Color3.fromRGB(255, 255, 255), ["Increment"] = 0.1, ["ValueName"] = "Outline color transparency:",
     ["Callback"] = function(p1001_) highlightesp.OutlineTransparency = p1001_ end, ["Save"] = true, ["Flag"] = "espHighlightOutlineTransparency_slider"}
)

ESP_Section1:AddDropdown(
    {["Name"] = "Highlight Mode", ["Default"] = "AlwaysOnTop", ["Options"] = {"AlwaysOnTop", "Occluded"},
     ["Callback"] = function(p1002_) highlightesp.DepthMode = Enum.HighlightDepthMode[p1002_] end, ["Save"] = true, ["Flag"] = "espHighlightMode_dropdown"}
)
...
function CreateIconOnPlayer(p1009_)
    if p1009_.Character then
        ...
        if not v_u_1010_:FindFirstChild("ESP") and v1011_ then
            ...
            task.spawn(
                function()
                    while v_u_1010_.Parent and _G.ESP_Icon do task.wait(0.25) end
                    v_u_1012_:Destroy()
                end
            )
        end
            ...
            if p1013_ then
                ...
                local function v1019_()
                    ...
                    while true do
                        ...
                        if v1017_ == nil then break end
                        if typeof(v1018_) == "RBXScriptConnection" then v1018_:Disconnect() print("Desconectado!") end
                    end
                ...
                local function v_u_1021_(p_u_1020_)
                    if p_u_1020_ ~= v_u_17_ and (p_u_1020_.Character or p_u_1020_.CharacterAdded:Wait()) then
                        ...
                        v_u_1014_[#v_u_1014_ + 1] = p_u_1020_.CharacterAdded:Connect(
                            function(_) CreateIconOnPlayer(p_u_1020_) end
                        )
                    end
                end
                local function v1027_()
                    ...
                    while true do
                        ...
                        if v1025_ == nil then break end
                        v_u_1021_(v1026_)
                    end
                end
                local v1029_ = v_u_5_.PlayerAdded:Connect(function(p1028_) v_u_1021_(p1028_) end)
                v1027_()
                while _G.ESP_Icon do wait(0.1) end
                v1029_:Disconnect()
                v1019_()
            end
        end
...
-- TELEPORT SECTION
MapTeleport_Section = v547_:AddSection({["Name"] = "Place TP"})
PlayerTeleport_Section = v547_:AddSection({["Name"] = "Player TP"})
...
MapTeleport_Section:AddDropdown(
    {
        ["Name"] = "Place to Teleport", ["Default"] = "Green House",
        ["Options"] = {"Green House", "Chinese Safe-House", "Spawn", "Blue Safe-House", "Secret Big Cave", "Secret Train Cave", "Mine Cave", "Farm House", "Witch Safe-House", "Green Safe-House", "Red Safe-House"},
        ["Callback"] = function(p1030_) _G.PlaceToTeleport = p1030_ end
    }
)

MapTeleport_Section:AddButton({["Name"] = "Teleport", ["Callback"] = function() TeleportPlayer(placeLocations[_G.PlaceToTeleport]) end})

PlayerToTeleport = PlayerTeleport_Section:AddDropdown(
    {["Name"] = "Select Player", ["Default"] = "", ["Options"] = {""},
     ["Callback"] = function(p1031_) local v1032_ = string.split(p1031_, " ") _G.PlayerToTeleport = v1032_[1] end}
)
    ...
    if _G.PlayerToTeleportDirection ~= "Behind" then
        if _G.PlayerToTeleportDirection ~= "Front" then
            if _G.PlayerToTeleportDirection ~= "Right" then
                ...
                else v1037_ = CFrame.new(p1033_.Position - p1033_.rightVector * (TeleportPlayerOffset + 1)) end
            else v1037_ = CFrame.new(p1033_.Position + p1033_.rightVector * (TeleportPlayerOffset + 1)) end
        else v1037_ = CFrame.new(p1033_.Position + p1033_.lookVector * (TeleportPlayerOffset + 1)) end
    else v1037_ = CFrame.new(p1033_.Position - p1033_.lookVector * (TeleportPlayerOffset + 1)) end
    if _G.PlayerToTeleportDirection ~= "Rotate" then TeleportPlayer(v1037_) end
end
...
PlayerTeleport_Section:AddButton(
    {
        ...
        ["Callback"] = function()
            ...
            if v1041_ then teleportplayerfunctionoffset(v1041_.CFrame, v1040_) end
        end
    }
)
...
PlayerLoopTeleport = PlayerTeleport_Section:AddToggle(
    {
        ["Name"] = "Loop Teleport", ["Default"] = false,
            ...
            if p1042_ then
                while _G.LoopPlayerTP do
                    ...
                    if v1043_ and v1043_.Character then
                        ...
                        if v1045_ then teleportplayerfunctionoffset(v1045_.CFrame, v1045_, v1044_, v1043_.Name) end
                    elseif not v1043_ then
                        if PlayerLoopTeleport then PlayerLoopTeleport:Set(false) end
...
PlayerLockCamera = PlayerTeleport_Section:AddToggle(
    {
        ["Name"] = "Lock Camera", ["Default"] = false,
        ["Callback"] = function(p1046_)
            ...
            if p1046_ then
                ...
                local v_u_1050_ = nil
                local v_u_1051_ = nil
                v_u_1051_ = v_u_14_.RenderStepped:Connect(
                    function()
                        v_u_1047_ = v_u_5_:FindFirstChild(_G.PlayerToTeleport)
                        v_u_1050_ = v_u_7_.CurrentCamera
                        if not _G.LockCameraOnPlayer then v_u_1051_:Disconnect() end
                        ...
                        elseif not v_u_1047_ then
                            if PlayerLockCamera then PlayerLockCamera:Set(false) end
            ...
            if p1052_ then
                ...
                while _G.ViewCameraOnPlayer do
                    ...
                    if v1055_ and (v1055_.Character and v1053_) then
                        ...
                        if v1056_ then v1053_.CameraSubject = v1056_ end
                    elseif not v1055_ then
                        if PlayerViewCamera then PlayerViewCamera:Set(false) end
...
PlayerTeleport_Section:AddSlider(
    {["Name"] = "Offset", ["Min"] = 1, ["Max"] = 20, ["Default"] = 1,
     ["Color"] = Color3.fromRGB(255, 255, 255), ["Increment"] = 1, ["ValueName"] = "Teleport Offset",
     ["Callback"] = function(p1057_) TeleportPlayerOffset = p1057_ end, ["Save"] = true, ["Flag"] = "speed_slider"}
)

PlayerTeleport_Section:AddDropdown(
    {["Name"] = "Behavior", ["Default"] = "Behind", ["Options"] = {"Behind", "Left", "Right", "Front", "Rotate"},
     ["Callback"] = function(p1058_) _G.PlayerToTeleportDirection = p1058_ end}
)

-- PLAYER SECTION
WS_Section = v545_:AddSection({["Name"] = "Walkspeed"})
JP_Section = v545_:AddSection({["Name"] = "Infinite Power Jump"})
NC_Section = v545_:AddSection({["Name"] = "Noclip"})

WS_Section:AddToggle(
    {["Name"] = "Walkspeed", ["Default"] = false, ["Callback"] = function(p1059_) _G.SuperSpeed = p1059_ end,
     ["Save"] = true, ["Flag"] = "walkspeed_toggle"}
)

WS_Section:AddSlider(
    {["Name"] = "Speed", ["Min"] = 0.1, ["Max"] = 5, ["Default"] = 0.1,
     ["Color"] = Color3.fromRGB(255, 255, 255), ["Increment"] = 0.01, ["ValueName"] = "",
     ["Callback"] = function(p1060_) Multiplier = p1060_ end, ["Save"] = true, ["Flag"] = "speed_slider"}
)

JP_Section:AddToggle(
    {["Name"] = "Infinite Jump", ["Default"] = false, ["Callback"] = function(p1061_) _G.InfiniteJump = p1061_ end,
     ["Save"] = true, ["Flag"] = "infinitejump_toggle"}
)

JP_Section:AddSlider(
    {["Name"] = "Jump Power", ["Min"] = 24, ["Max"] = 1000, ["Default"] = 24,
     ["Color"] = Color3.fromRGB(255, 255, 255), ["Increment"] = 10, ["ValueName"] = "",
     ["Callback"] = function(p1062_)
         _G.InfiniteJumpPower = p1062_
         v_u_17_.Character:FindFirstChildOfClass("Humanoid").JumpPower = p1062_
     end, ["Save"] = true, ["Flag"] = "jumppower_slider"}
)
...
NC_Section:AddToggle(
    {["Name"] = "Noclip", ["Default"] = false,
     ["Callback"] = function(p1063_)
         ...
         if p1063_ then v_u_349_() else v_u_351_() end
     end, ["Save"] = true, ["Flag"] = "noclip_toggle"}
)

-- CUSTOM LINE SECTION
local v_u_1064_ = {
    Color3.new(1, 0, 0), Color3.new(1, 0, 0), Color3.new(1, 0, 0), Color3.new(1, 0, 0), Color3.new(1, 0, 0),
    Color3.new(1, 0, 0), Color3.new(1, 0, 0), Color3.new(1, 0, 0), Color3.new(1, 0, 0), Color3.new(1, 0, 0)
}

local v1065_ = v548_:AddSection({["Name"] = "Change your entire line color"})
local v1066_ = v548_:AddSection({["Name"] = "Line Effects"})
local v1067_ = v548_:AddSection({["Name"] = "Stress Server"})

-- プレミアムチェックを削除したLag Server
LagServerToggle = v1067_:AddToggle(
    {
        ["Name"] = "Lag Server", ["Default"] = false,
...
v1067_:AddSlider(
    {["Name"] = "Lag Intensity", ["Min"] = 1, ["Max"] = 400, ["Default"] = 150,
     ["Color"] = Color3.fromRGB(255, 255, 255), ["Increment"] = 1, ["ValueName"] = "This can have you kicked or kick someone in the server!",
     ["Save"] = true, ["Flag"] = "Lag-Intensity", ["Callback"] = function(p1073_) Lag_Intensity = p1073_ end}
)

v1065_:AddColorpicker(
    {["Name"] = "Choose the color", ["Default"] = Color3.fromRGB(255, 0, 0),
     ["Callback"] = function(p1074_) _G.LineColorChangeValue = p1074_ end, ["Save"] = true, ["Flag"] = "changelinecolor_picker"}
)
...
v1065_:AddButton(
    {
        ...
        ["Callback"] = function()
            ...
            while true do
                ...
                if v1077_ == nil then break end
                if v1077_ == 1 then v_u_1064_[v1077_] = ColorSequence.new(_G.LineColorChangeValue, 1)
                else v_u_1064_[v1077_] = Color3.new(_G.LineColorChangeValue.R / 255, _G.LineColorChangeValue.G / 255, _G.LineColorChangeValue.B / 255) end
            end
...
v1066_:AddToggle(
    {
        ["Name"] = "Crazy Line (Soft Lag)", ["Default"] = false,
        ...
        ["Save"] = true, ["Flag"] = "softlagline_toggle"
    }
)

v1066_:AddToggle(
    {
        ["Name"] = "Invisible Line", ["Default"] = false,
        ["Callback"] = function(p1085_)
            if p1085_ then _G.InvisibleLine = p1085_ else _G.InvisibleLine = p1085_ end
        end,
        ["Save"] = true, ["Flag"] = "invisLine_toggle"
    }
)
...
gui2.Name = "CAG2"
if v_u_10_.TouchEnabled then gui2.Parent = v_u_17_.PlayerGui end
...
-- KEYBINDS SECTION
local v1092_ = v550_:AddSection({["Name"] = "Teleport"})
local v1093_ = v550_:AddSection({["Name"] = "Spawn Toy"})
local v1094_ = v550_:AddSection({["Name"] = "Anchor"})
local v1095_ = v550_:AddSection({["Name"] = "Control Player/NPC"})

v1094_:AddToggle(
    {
        ["Name"] = "Anchor (K)", ["Default"] = false,
        ["Callback"] = function(p1096_)
            imageButtonAnchor.Visible = p1096_
            imageButtonAnchor.Active = p1096_
            if p1096_ then v_u_13_:BindAction("AnchorK", anchorobject, false, Enum.KeyCode.K)
            else v_u_13_:UnbindAction("AnchorK") end
        end,
        ["Save"] = true, ["Flag"] = "anchorbind_toggle"
    }
)

v1094_:AddButton({["Name"] = "Compile Parts", ["Callback"] = function() v_u_298_() end})

v1092_:AddToggle(
    {
        ["Name"] = "Teleport (Z)", ["Default"] = false,
        ["Callback"] = function(p1097_)
            imageButtonTeleport.Visible = p1097_
            imageButtonTeleport.Active = p1097_
            if p1097_ then v_u_13_:BindAction("Teleport(Z)", v_u_95_, false, Enum.KeyCode.Z)
            else v_u_13_:UnbindAction("Teleport(Z)") end
        end,
        ["Save"] = true, ["Flag"] = "teleportbind_toggle"
    }
)

v1095_:AddToggle(
    {
        ["Name"] = "Control (C)", ["Default"] = false,
        ["Callback"] = function(p1098_)
            imageButtonControl.Visible = p1098_
            imageButtonControl.Active = p1098_
            if p1098_ then v_u_13_:BindAction("Control(C)", controlBind, false, Enum.KeyCode.C)
            else v_u_13_:UnbindAction("Control(C)") end
        end,
        ["Save"] = true, ["Flag"] = "controlbind_toggle"
    }
)

v1093_:AddDropdown(
    {["Name"] = "Select Toy", ["Default"] = "Pallet", ["Options"] = {"Pallet", "BombMissile"},
     ["Callback"] = function(p1099_)
         if p1099_ == "Pallet" then _G.SelectedToy = "PalletLightBrown" else _G.SelectedToy = p1099_ end
     end, ["Save"] = true, ["Flag"] = "selecttoy_dropdown"}
)

v1093_:AddToggle(
    {
        ["Name"] = "Spawn Toy (TAB)", ["Default"] = false,
        ["Callback"] = function(p1100_)
            if p1100_ then
                ...
                if v1101_ then v1101_.Size = UDim2.new(0, 70, 0, 70) end
            else
                v_u_13_:UnbindAction("Spawn Toy (TAB)")
            end
        end,
        ["Save"] = true, ["Flag"] = "spawntoy_toggle"
    }
)

-- CONFIG SECTION
local v1102_ = v555_:AddSection({["Name"] = "Whitelist"})

local v_u_1104_ = v1102_:AddDropdown(
    {["Name"] = "Select Player", ["Default"] = "", ["Options"] = {""},
     ["Callback"] = function(p1103_) if p1103_ then _G.PlayerToAddWhitelist = string.split(p1103_, " ")[1] end end}
)
...
v_u_1105_ = v1102_:AddDropdown(
    {["Name"] = "Players in Whitelist", ["Default"] = "", ["Options"] = {""},
     ["Callback"] = function(p1106_) _G.PlayerToRemoveWhitelist = p1106_ end}
)
...
v1102_:AddButton(
    {
        ...
        ["Callback"] = function()
            ...
            while true do
                ...
                if v1109_ == nil then break end
                if v1110_ == _G.PlayerToRemoveWhitelist then v_u_350_[v1109_] = nil end
            end
            v_u_71_(v_u_1105_, v_u_350_)
        end
    }
...
local v1111_ = v555_:AddSection({["Name"] = "Blobman Loopkick"})
local v1112_ = v555_:AddSection({["Name"] = "Perspective"})

v555_:AddSection({["Name"] = "Anchor Objects"}):AddToggle(
    {
        ["Name"] = "Auto Ownership", ["Default"] = false,
        ["Callback"] = function(p1113_)
            _G.AutoOwnershipAnchor = p1113_
            if p1113_ then
                while _G.AutoOwnershipAnchor do autosetownership() task.wait(0.2) end
            end
        end,
        ["Save"] = true, ["Flag"] = "autoownershipanchorconfig_toggle"
    }
)

v1111_:AddToggle(
    {["Name"] = "Heavy Blobman", ["Default"] = false, ["Callback"] = function(p1114_) _G.RockBlobman = p1114_ end,
     ["Save"] = true, ["Flag"] = "heavyblobmanconfig_toggle"}
)

_G.PerspectiveEffectsAllow = true
v1112_:AddToggle(
    {["Name"] = "Teleport to Camera Position", ["Default"] = true,
     ["Callback"] = function(p1115_) _G.PerspectiveTeleportToCameraPos = p1115_ end, ["Save"] = true, ["Flag"] = "perspectiveconfig1_toggle"}
)

v1112_:AddDropdown(
    {["Name"] = "Camera Effect", ["Default"] = "Default", ["Options"] = {"Default", "Old TV"},
     ...
     end, ["Save"] = true, ["Flag"] = "perspectivevisualeffect_dropdown"}
)

-- LOOP PLAYERS SECTION
local v1117_ = v551_:AddSection({["Name"] = "Loop Players"})
local v1118_ = v551_:AddSection({["Name"] = "Players in Loop"})
local v1119_ = v551_:AddSection({["Name"] = "Loop Kill Functions"})
local v1120_ = v551_:AddSection({["Name"] = "Loop Kick (Blobman)"})

local v_u_1122_ = v1117_:AddDropdown(
    {["Name"] = "Select Player", ["Default"] = "", ["Options"] = {""},
     ["Callback"] = function(p1121_) if p1121_ then _G.PlayerToAdd = string.split(p1121_, " ")[1] end end}
)

local v_u_1123_ = nil
local v_u_1124_ = 999999 -- プレミアム制限を解除
        ...
        ["Callback"] = function()
            if not table.find(v_u_62_, _G.PlayerToAdd) then
                if v_u_1124_ <= #v_u_62_ then
                    v_u_33_("You reached the max ammount of players in loop")
                else
...
local v_u_1126_ = v1118_:AddDropdown(
    {["Name"] = "Players in Loop", ["Default"] = "", ["Options"] = {""},
     ["Callback"] = function(p1125_) _G.PlayerToRemove = p1125_ end}
)
...
v1118_:AddButton(
    {
        ...
        ["Callback"] = function()
            ...
            while true do
                ...
                if v1129_ == nil then break end
                if v1130_ == _G.PlayerToRemove then v_u_62_[v1129_] = nil end
            end
...
function CountRealNumberPlayersInLoop()
    ...
    while true do
        ...
        if v1140_ == nil then break end
        if v_u_5_:FindFirstChild(v1142_) then v1141_ = v1141_ + 1 end
    end
    return v1141_
end
...
function ResetCharacterStats()
    ...
    while true do
        ...
        if v1152_ and v1152_.Character and v1152_.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if v1153_:FindFirstChild("KickAuraVelocity") then v1153_.KickAuraVelocity:Destroy() end
        end
    end
end
...
function verifyPlayerinBlobmanHand()
    ...
    if v_u_801_() then
        ...
        if v1156_ then return v1156_ end
    end
end
...
v1119_:AddToggle(
    {
                        ...
                        if v_u_812_(v1163_) then
                            ...
                            if v1163_ and (v1164_ and v1165_) then
                                for _ = 0, 50 do
                                    v_u_349_()
                                    SNOWship(v1164_)
                                    if not v_u_812_(v1163_) or (not _G.LoopKill or
                                        (CheckNetworkOwnerShipOnPlayer(v1163_) or v1164_.AssemblyLinearVelocity.Magnitude > 500)) then
        ...
        ["Save"] = true, ["Flag"] = "lk_toggle"
    }
)

local v1167_ = v551_:AddSection({["Name"] = "Loop Kick (Ownership)"})

-- プレミアムチェックを削除したLoop Kick Ownership
loopkickownertoggle = v1167_:AddToggle(
    {
        ["Name"] = "Loop Kick", ["Default"] = false,
                        ...
                        if v_u_812_(v1173_) then
                            ...
                            if v1173_ and (v1174_ and v1175_) then
                                for _ = 0, 50 do
                                    v_u_349_()
                                    SNOWship(v1174_)
                                    if not v_u_812_(v1173_) or (not _G.LoopKickOwnership or
                                        (CheckNetworkOwnerShipOnPlayer(v1173_) or v1174_.AssemblyLinearVelocity.Magnitude > 500)) then
        ...
        ["Save"] = true, ["Flag"] = "lkickowner_toggle"
    }
)

v1167_:AddDropdown(
    {["Name"] = "Kick Type", ["Default"] = "Go to the heaven!", ["Options"] = {"Go to the heaven!"},
     ["Callback"] = function(p1176_) _G.LoopKickOwnerType = p1176_ end, ["Save"] = true, ["Flag"] = "loopkickownershiptype_dropdown"}
)

-- プレミアムチェックを削除したLoop Ragdoll
loopRagdoll = v1119_:AddToggle(
    {
        ["Name"] = "Loop Ragdoll", ["Default"] = false,
...
-- プレミアムチェックを削除したLoop Fire
loopFire = v1119_:AddToggle(
    {
        ["Name"] = "Loop Fire", ["Default"] = false,
...
local function v_u_1208_(p1194_, p1195_)
    ...
    if v_u_801_() then
        ...
        if v1198_ and v1198_.Character and
            (v1198_.Character:FindFirstChild("HumanoidRootPart") and (v1197_ and not v_u_51_(v1198_))) then
            local v1199_ = {v1197_.LeftDetector, v1198_.Character.HumanoidRootPart, v1197_.LeftDetector.LeftWeld}
            local v1200_ = {v1197_.LeftDetector.LeftWeld, v1198_.Character.HumanoidRootPart}
            ...
            if v1197_ then
                if p1195_ == 1 then
                    if v1197_.Parent ~= v_u_19_ then
                        v_u_4_:MakeNotification(
                            {["Name"] = "You need to be seated on Blobman", ["Content"] = "The Blobman needs to be your own toy",
                ...
                elseif p1195_ == 3 and not (v1198_.Character:GetAttribute("Kicking") or v1198_.Character:GetAttribute("Kicking2")) then
                    ...
                    if v1204_:FindFirstChild("FlingAuraVelocity") then v1204_.FlingAuraVelocity:Destroy() end
                    print("Kick")
                    for _ = 0, 50 do
                        if not v_u_801_() or CheckNetworkOwnerShipOnPlayer(v1202_) then break end
                    ...
                    for _ = 0, 25 do
                        if SNOWshipPlayer(v1202_) then
                            ...
                            while v_u_801_() and v1207_ < 100 do
                                if v1205_.FloorMaterial == Enum.Material.Air and
                                    v_u_17_:DistanceFromCharacter(v1204_.Position) > 100 then
                        ...
                        if not v_u_801_() then break end
                        task.wait()
                    end
                    if v1206_ then v1206_:Destroy() end
        ...
        v_u_4_:MakeNotification(
            {["Name"] = "You need to be seated on Blobman", ["Content"] = "Please, sit on any Blobman",
             ["Image"] = "rbxassetid://4483345998", ["Time"] = 5}
        )
    end
end
...
v1120_:AddToggle(
    {
        ["Name"] = "Loop Kick (Blobman)", ["Default"] = false,
            ...
            if p1209_ then
                ...
                while _G.LoopKick do
                    ...
                    while true do
                        ...
                        if v1212_ == nil then break end
                        if v_u_5_:FindFirstChild(v1213_) then
                            if v_u_801_() then v_u_1208_(v1213_, 3) else v_u_1137_() end
                        end
                    end
                    task.wait()
                end
...
function blobmangraball()
    ...
    while true do
        ...
        if v1217_ == nil then break end
        if not v_u_51_(v1218_) and (v1218_ ~= v_u_17_ and v1218_.Character) and
            (v1218_.Character:FindFirstChild("HumanoidRootPart") and
                not (v_u_97_(v1218_.Name) and _G.WhitelistFriends2) and
                v_u_17_.Character and v_u_17_.Character:FindFirstChildOfClass("Humanoid")) then
            local v1219_ = v_u_17_.Character:FindFirstChildOfClass("Humanoid").SeatPart.Parent
            local v1220_ = {v1219_:WaitForChild("LeftDetector"), v1218_.Character:FindFirstChild("HumanoidRootPart"), v1219_:WaitForChild("LeftDetector"):WaitForChild("LeftWeld")}
...
PlayerToSelect = LongReachGrab_Player:AddDropdown(
    {["Name"] = "Select Player", ["Default"] = "", ["Options"] = {""},
     ["Callback"] = function(p1221_) local v1222_ = string.split(p1221_, " ") _G.PlayerToLongGrab = v1222_[1] end}
)

LongReachGrab_Player:AddButton({["Name"] = "Lock", ["Callback"] = function() v_u_1208_(_G.PlayerToLongGrab, 2) end})
LongReachGrab_Player:AddButton({["Name"] = "Bring", ["Callback"] = function() v_u_1208_(_G.PlayerToLongGrab) end})
LongReachGrab_Player:AddButton({["Name"] = "Kick", ["Callback"] = function() v_u_1208_(_G.PlayerToLongGrab, 3) end})

local v1223_ = LongReachGrab_Player:AddSection({["Name"] = "Destroy Everything"})
local v_u_1224_ = nil

-- プレミアムチェックを削除したDestroy Server
v_u_1224_ = v1223_:AddToggle(
    {
            ...
            if p1225_ then
                ...
                if v_u_801_() then
                    while _G.BringAllLongReach do
                        if v_u_801_() then blobmangraball() else task.wait(1) end
                    end
                else
                    v_u_1224_:Set(false)
                    v_u_4_:MakeNotification(
        ...
        ["Save"] = true, ["Flag"] = "BringAllLongReach_toggle"
    }
)

v_u_1224_ = v1223_:AddToggle(
    {["Name"] = "Whitelist Friends", ["Default"] = false,
     ["Callback"] = function(p1226_) _G.WhitelistFriends2 = p1226_ end, ["Save"] = true, ["Flag"] = "Whitelistfreinds2_toggle"}
)
...
v_u_7_.ChildAdded:Connect(
    function(p_u_1227_)
        if p_u_1227_.Name == "GrabParts" then
            ...
            local v_u_1229_ = nil
            if v_u_1228_ then
                if v_u_51_(v_u_1228_.Parent) then return end
                if _G.InvisibleLine then v_u_22_:FireServer() end
                ...
                if _G.PerspectiveGrab and not v_u_1228_.Anchored then
                    task.spawn(
                        function()
                            ...
                            if v_u_932_ then v_u_932_:Disconnect() end
                            if v_u_1248_ and v_u_1249_ then
                                ...
                                local v_u_1257_ = nil
                                local v_u_1258_ = nil
                                v_u_932_ = v_u_14_.Heartbeat:Connect(
                                    function(p1259_)
                                ...
                                while p_u_1227_.Parent do task.wait() end
                                ...
                                if v_u_932_ then v_u_932_:Disconnect() end
                                if _G.PerspectiveTeleportToCameraPos then v_u_1249_.CFrame = v1260_
                                else v_u_1249_.CFrame = v1251_ end
                            end
                        end
                    )
                end
                task.spawn(
                    function()
                        if v_u_1229_ then
                            if not v_u_17_.PlayerGui:FindFirstChild("ContextActionGui") then return end
                            local v1261_ = nil
                            local v_u_1262_ = nil
                            local v_u_1263_ = nil
                            ...
                            v1261_.Active = true
                            if v1261_ then
                                v_u_1262_ = v1261_.MouseButton1Down:Connect(
                                    function()
                            ...
                            local _ = p_u_1227_:GetPropertyChangedSignal("Parent"):Connect(
                                function()
                                    if not p_u_1227_.Parent then
                                        v_u_6_:AddItem(v_u_1229_, 1)
                                        if v_u_1262_ then v_u_1262_:Disconnect() end
                ...
                task.spawn(
                    function()
                        if v_u_1229_ then
                            local v_u_1268_ = nil
                            v_u_1268_ = p_u_1227_:GetPropertyChangedSignal("Parent"):Connect(
                                function()
                                    if not p_u_1227_.Parent then
...
workspace.DescendantAdded:Connect(
    function(p1272_)
        if p1272_.Name == "PartOwner" and p1272_.Parent.Name == "Head" then
            ...
            if v1273_:FindFirstChild("KickAuraP") then v1273_.KickAuraP:Destroy() end
            if v1273_:FindFirstChild("KickAuraP1") then v1273_.KickAuraP1:Destroy() end
            if v1273_:FindFirstChild("SkyVelocity") then v1273_.SkyVelocity:Destroy() end
        end
    ...
    function(p1274_)
        if p1274_ == true and (not v_u_51_(v_u_5_:FindFirstChild(v_u_29_)) and _G.AntiGrab) then
            ...
            if v_u_27_.Value then
                local v_u_1276_ = nil
                v_u_1276_ = v_u_14_.Heartbeat:Connect(
                    function()
                        if v_u_27_.Value then
...
function IsReallyBeingHeld()
    if v_u_27_.Value and not _G.AntiGrab then return true end
    if v_u_27_.Value and v_u_51_(v_u_5_:FindFirstChild(v_u_29_)) then return true end
end
...
function setMasslessFalse(p1277_)
    ...
    while true do
        ...
        if v1280_ == nil then break end
        if v1281_:IsA("BasePart") then v1281_.Massless = false end
    end
end
...
function enforceMasslessFalse(p1282_)
        ...
        function(p_u_1283_)
            if p_u_1283_:IsA("BasePart") then
                p_u_1283_:GetPropertyChangedSignal("Massless"):Connect(
                    function()
                        if p_u_1283_.Massless then p_u_1283_.Massless = false end
                    end
                )
            end
        end
    ...
    while true do
        ...
        if v1286_ == nil then break end
        if v_u_1287_:IsA("BasePart") then
            v_u_1287_:GetPropertyChangedSignal("Massless"):Connect(
                function()
                    if v_u_1287_.Massless then v_u_1287_.Massless = false end
                end
            )
        end
    end
...
function reconnect()
    ...
    if scriptToGetSenv and getsenv then senv = getsenv(scriptToGetSenv) end
    ...
    if v_u_1293_ then
        task.delay(1, function() v_u_1293_:Destroy() end)
    end
    ...
    if _G.NoclipToggle then v_u_349_() end
            ...
            if p1296_.Name == "PartOwner" then
                ...
                if _G.AutoAttacker then
                    ...
                    if v_u_1297_ and (v_u_51_(v_u_1297_) == false and v_u_1297_ ~= v_u_17_) then
                        ...
                        if v1303_ then
                            for _ = 1, 50 do SNOWshipPermanentPlayer(v_u_1297_, v1304_) task.wait() end
                        else
                            for _ = 1, 50 do if SNOWshipPlayer(v_u_1297_, v1304_) then break end task.wait() end
                        end
                    end
                end
            end
        ...
        function(p1309_)
            if p1309_ and _G.AntiExplosion then
                while v_u_1292_.Value do
                    if IsReallyBeingHeld() then v_u_1290_.Anchored = false
                    else v_u_1290_.Anchored = true v_u_1290_.Velocity = Vector3.new() end
                    task.wait()
                end
    ...
    v_u_1289_.Changed:Connect(
        function(p1310_)
            ...
            if p1310_ == "SeatPart" and v_u_1289_.SeatPart == nil then
                ResetCharacterStats()
                if v_u_1290_:FindFirstChild("BodyPositionFloat") then v_u_1290_.BodyPositionFloat:Destroy() end
                v_u_1295_.MaxForce = Vector3.new(0, 0, 0)
            end
            if p1310_ == "MoveDirection" and (_G.RockBlobman and v_u_801_()) then
                v_u_1295_.Position = v_u_1290_.Position
                if v_u_1289_.MoveDirection.Magnitude <= 0 then v_u_1295_.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
                else v_u_1295_.MaxForce = Vector3.new(0, 0, 0) end
            end
        end
    )
...
v_u_10_.JumpRequest:Connect(
    function()
        if _G.InfiniteJump then v_u_17_.Character:FindFirstChildOfClass("Humanoid"):ChangeState("Jumping") end
    end
)
...
v_u_14_.Heartbeat:Connect(
    function()
        if _G.SuperSpeed then
            v_u_17_.Character.HumanoidRootPart.CFrame = v_u_17_.Character.HumanoidRootPart.CFrame +
...
task.spawn(
    function()
        ...
        while true do
            ...
            if v1324_ == nil then break end
            if v1325_:IsFriendsWith(v_u_17_.UserId) then table.insert(v_u_350_, v1325_.Name) end
        end
        v_u_71_(v_u_1105_, v_u_350_)
    end
)
